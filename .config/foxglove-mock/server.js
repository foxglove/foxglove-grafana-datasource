...
const server = http.createServer(async (req, res) => {
  ...
  // Minimal endpoints used by the plugin backend.
  if (req.method === 'GET' && path === '/v1/devices') {
    return sendJson(res, 200, { devices: [] });
  }

  if (req.method === 'POST' && path === '/v1/data/stream') {
    // The plugin sends JSON, but we don't need it.
    await readBody(req);
    return sendJson(res, 200, { url: 'http://foxglove-mock:8080/mock-download' });
  }

  if (req.method === 'GET' && path === '/mock-download') {
    // The backend expects a JSON blob with `messages: [{timestamp, value}, ...]`.
    // Use a tiny deterministic response so e2e can succeed without external deps.
