...
- The filter editor accepts the same filter text as Foxglove Search (`@device.name == husky`, `/battery.percentage < 20`, `and` / `or`), including `contains`, `exists`, episode, and session filters. `in` remains, written as `@device.name in a,b`. Saved condition trees are shown as text and run through that text. Rows with an empty value are omitted, so they no longer empty an AND group. A row with a value but an empty message path fails the query.
